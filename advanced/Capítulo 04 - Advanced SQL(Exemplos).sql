...
WHERE 1=2; -- para criar a tabela vazia
...
INSERT  ALL 
INTO sal_hist_dept VALUES(employee_id ,hire_date   ,salary ) 
INTO mgr_hist_dept VALUES(employee_id ,manager_id  ,salary )
SELECT employee_id , 
       hire_date   ,
       salary , 
       manager_id  
...
INSERT 
  ...
  WHEN MANAGER IS NULL   THEN 
    INTO mgr_hist_dept VALUES(EMP_ID,MANAGER,SALARY) --aguenta até mais ou menos 1500 tabelas, precisa olhar a doc
...
INSERT FIRST
 ...
 select * from  highest_sal; --90, 60, 110, 80, 50, 100
 select * from  hiredate_hist_00; --ninguém, porque já inseriu na primeira tabela
 select * from  hiredate_hist_99; --null, 30
 select * from  hiredate_hist; --20, 70, 10, 40
/*
10	4400	17/09/87 -3
40	6500	07/06/94 -3
	7000	24/05/99 -2
...
SELECT employee_id,
       last_name,
       week_id,
       monday+tuesday+wednesday+thursday+friday sum_sales
FROM   sales_data;

SELECT employee_id,
       last_name,
       week_id,
       (monday+tuesday+wednesday+thursday+friday)/5 avg
FROM   sales_data;

SELECT employee_id, last_name,
       MAX(pivo_por_union.vl_venda)
FROM   (SELECT employee_id, last_name, monday vl_venda
        FROM   SALES_DATA
        UNION ALL
        SELECT employee_id, last_name, tuesday
        FROM   SALES_DATA
        UNION ALL
        SELECT employee_id, last_name, wednesday
        FROM   SALES_DATA
        UNION ALL
        SELECT employee_id, last_name, thursday
        FROM   SALES_DATA
        UNION ALL
        SELECT employee_id, last_name, friday
        FROM   SALES_DATA) pivo_por_union
WHERE   pivo_por_union.employee_id = 176
GROUP   BY (pivo_por_union.employee_id, pivo_por_union.last_name);
...
SELECT department_name, e.job_id, sum(e.salary)
FROM   employees e
JOIN   departments d
...
JOIN   departments d
ON     d.department_id = e.department_id
GROUP  BY GROUPING SETS((d.department_name, e.job_id), (d.department_name), ()) --visões diferentes
ORDER  BY 1, 2;
...
JOIN   departments d
ON     d.department_id = e.department_id
GROUP  BY GROUPING SETS((d.department_name, e.job_id), (d.department_name), (e.job_id) ())
ORDER  BY 1, 2;
